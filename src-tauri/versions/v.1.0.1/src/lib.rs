...
#[tauri::command]
fn save_equation_to_md(content: String, label: String, state: tauri::State<'_, AppState>) -> Result<String, String> {
    println!("DEBUG: save_equation_to_md called with label: {}", label);

    // Clone the string so we don't hold the lock during file I/O
    let workspace_path = state.workspace_path.lock().map_err(|_| "Failed to lock state")?.clone();
    
    println!("DEBUG: Current workspace_path in state is: '{}'", workspace_path);

    // Safety fallback: if state is empty, try to get the path from the saved config
    let target_dir = if workspace_path.is_empty() {
        let config = load_config();
        if !config.theory_md_dir.is_empty() {
            println!("DEBUG: State empty. Falling back to theory_md_dir: '{}'", config.theory_md_dir);
            config.theory_md_dir
        } else if !config.project_root_dir.is_empty() {
            println!("DEBUG: State empty. Falling back to project_root_dir: '{}'", config.project_root_dir);
            config.project_root_dir
        } else {
            return Err("Workspace path is empty. Please set your root directory in settings.".to_string());
        }
    } else {
        workspace_path
    };

    let file_path = std::path::Path::new(&target_dir).join("theories.md");
    println!("DEBUG: Attempting to write to: {:?}", file_path);
    ...
    println!("DEBUG: Successfully saved to {:?}", file_path);
    Ok(format!("Successfully saved {} to theories.md", label))
}
...
#[tauri::command]
fn save_root_directory(path: &str, state: tauri::State<'_, AppState>) -> Result<String, String> {
    // This line updates the state for your save_equation_to_md command
    let mut workspace = state.workspace_path.lock().unwrap();
    *workspace = path.to_string();

    // ... your existing config saving logic follows
...
pub fn run() {
    tauri::Builder::default()
        .manage(AppState {
            workspace_path: std::sync::Mutex::new(String::from("")), 
        })
